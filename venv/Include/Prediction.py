...
if __name__ == '__main__':
    ...
    p = Prediction()
    # p.parse_poll(test_str)
    p.parse_poll(test_str2)
    # p.parse_poll(test_str3)
    p.add_user_prediction("bob", "yes", 500)
    p.add_user_prediction("bill", "yes", 1000)
    p.add_user_prediction("bell", "no", 450)
    p.add_user_prediction("bogman", "no", 6000)
    p.add_user_prediction("larry", "maybe", 5000)
    print(str(p.get_split_totals()))
    # print(p.user_prediction)
    print(str(p.get_prediction_users("yes")))
    print(str(p.get_winners("yes")))

    #p.get_prediction_amount("no")
    #print(p.get_split_users())
    #print(p.user_prediction)
    #print(p)
    #print(p.get_pool())
    #print(p.get_prediction_totals("yes"))
  #  p.reset_poll()
    #print(p)
   # q2 = ["maybe", "that's a roger dodger"]
   # p2 = Prediction("Will we lose", q2)
   # print(p2)
    #p2.parse_poll("we will we win? /:/jellyJam /:/nopBob /:/maybert")
  #  print(p2)
